...
const thisShapesTagLength = useStorage((root) => root.shapes[SHAPE_INDEX].tags.length);
...
function Canvas() {
  ...
  const shapeIds = useStorage((root) => root.shapes.map((shape) => shape.id), shallow);
...
function Shape({ id }: { id: string }) {
  ...
  const shape = useStorage((root) => root.shapes.find((shape) => shape.id === id), shallow);
