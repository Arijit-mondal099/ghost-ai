// ---------------------------------------------------------------------------
// POST /api/ai/spec/token — mint a run-scoped Trigger.dev public token
//
...
//      expirationTime: "1hr" })` — a scopeless token authorizes nothing, so
//      the scope is mandatory; the 1-hour expiry is the spec-29 requirement
//      (the design token route uses the 15-minute default).
...
export async function POST(request: Request): Promise<Response> {
  ...
  const parsed = parseSpecTokenBody(body);
  if (!parsed.ok) {
